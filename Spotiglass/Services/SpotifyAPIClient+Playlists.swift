...
extension SpotifyAPIClient {
    ...
    func playlistTracks(playlistID: String, limit: Int = 50, maxPages: Int = 200) async throws
        -> [SpotifyPlaylistTrackItem]
    {
        ...
        return try await collectPaged(path: "/v1/playlists/\(playlistID)/items", limit: limit, maxPages: pageCap) {
            (dto: SpotifyPlaylistTrackItemDTO, index) in
