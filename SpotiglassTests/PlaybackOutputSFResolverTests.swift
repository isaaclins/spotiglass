...
final class PlaybackOutputSFResolverTests: XCTestCase {
    func testAirPodsMaxKeywordOrdering() {
        let candidates = PlaybackOutputSFResolver.symbolCandidates(deviceName: "Isaacs AirPods Max", spotifyDeviceType: nil)
        XCTAssertEqual(candidates.first, "airpodsmax")
    }

    func testHomePodMiniBeforeHomePod() {
        let mini = PlaybackOutputSFResolver.symbolCandidates(deviceName: "Living Room HomePod mini", spotifyDeviceType: nil)
    ...
    func testUnknownInputEndsWithHeadphonesCandidate() {
        let candidates = PlaybackOutputSFResolver.symbolCandidates(deviceName: "Obscure Bluetooth Thing", spotifyDeviceType: nil)
        XCTAssertEqual(candidates, ["headphones"])
    }

    func testLeftEarHintUsesSideVariantsFirst() {
        let candidates = PlaybackOutputSFResolver.symbolCandidates(deviceName: "AirPods Pro (Left)", spotifyDeviceType: nil)
