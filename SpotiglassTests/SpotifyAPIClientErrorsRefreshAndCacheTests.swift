...
final class SpotifyAPIClientErrorsRefreshAndCacheTests: XCTestCase {
    func testBadRequestMapsToBadRequestCaseWithDiagnostics() async throws {
        ...
        do {
            ...
            guard case .badRequest(let message, let details) = error else {
    ...
    func testErrorMappingCoversRateLimitForbiddenAuthDecodeAndNetwork() async throws {
        ...
        await XCTAssertThrowsSpotifyAPIError(
            try await rateLimitedClient.currentUserProfile(), .rateLimited(retryAfter: 7))
        ...
        let forbiddenClient = SpotifyAPIClient(
            tokenProvider: StaticSpotifyAccessTokenProvider(token: "token"), httpClient: forbidden)
        do {
            _ = try await forbiddenClient.currentUserProfile()
            XCTFail("Expected forbidden")
        } catch let error as SpotifyAPIError {
            guard case .forbidden(let message, let details) = error else {
        ...
        let insufficientScope = QueueHTTPClient([
            .json(
                ...
                headers: [
                    "WWW-Authenticate":
                        "Bearer realm=\"spotify\", error=\"insufficient_scope\", scope=\"playlist-read-private playlist-read-collaborative\""
                ])
        ])
        let insufficientScopeClient = SpotifyAPIClient(
            tokenProvider: StaticSpotifyAccessTokenProvider(token: "token"), httpClient: insufficientScope)
        do {
            _ = try await insufficientScopeClient.playlistTracks(playlistID: "playlist-1")
            XCTFail("Expected insufficient scope")
        } catch let error as SpotifyAPIError {
            guard case .insufficientScope(let requiredScopes, let message, let details) = error else {
            ...
            XCTAssertTrue(
                details?.contains("GET https://api.spotify.com/v1/playlists/playlist-1/items?limit=50&offset=0") == true
            )
            XCTAssertTrue(details?.contains("HTTP 403") == true)
        ...
        let genericForbiddenPlaylistTracksClient = SpotifyAPIClient(
            tokenProvider: StaticSpotifyAccessTokenProvider(token: "token"), httpClient: genericForbiddenPlaylistTracks)
        do {
        ...
        let badJSONClient = SpotifyAPIClient(
            tokenProvider: StaticSpotifyAccessTokenProvider(token: "token"), httpClient: badJSON)
        do {
    ...
    func testUnauthorizedRefreshRetriesWithFreshToken() async throws {
        ...
        XCTAssertEqual(
            httpClient.requests.map { $0.value(forHTTPHeaderField: "Authorization") },
            [
                "Bearer stale-token",
                "Bearer fresh-token",
            ])
    ...
    func testCachePersistsAndInvalidatesByAgeAndSnapshot() throws {
        ...
        XCTAssertEqual(
            try cache.loadTracks(
                playlistID: "playlist-1", snapshotID: "snapshot-1", now: Date(timeIntervalSince1970: 1_100), maxAge: 300
            ), [track])
        XCTAssertNil(
            try cache.loadTracks(
                playlistID: "playlist-1", snapshotID: "snapshot-2", now: Date(timeIntervalSince1970: 1_100), maxAge: 300
            ))
