...
final class SpotifyAPIClientSearchRateLimitAndGETRetryTests: XCTestCase {
    ...
    func testSearchSecondCallUsesGETResponseCacheSingleHTTPRequest() async throws {
        ...
        XCTAssertEqual(
            httpClient.requests.count, 1, "GET response cache should avoid a second HTTP request for the same search.")
    ...
    func testSearchCacheSharesKeyForCaseAndWhitespaceVariantsOfQ() async throws {
        ...
        XCTAssertEqual(
            httpClient.requests.count, 1, "Normalized cache keys should treat equivalent q parameters as one entry.")
    }
    ...
    func testRetryAfterHeaderSupportsHTTPDateFormat() async throws {
        ...
        let httpClient = QueueHTTPClient([
            .json(
                #"{"error":{"status":429,"message":"Slow"}}"#, statusCode: 429, headers: ["Retry-After": headerValue]),
            .json(
                #"{"error":{"status":429,"message":"Slow"}}"#, statusCode: 429, headers: ["Retry-After": headerValue]),
            .json(
                #"{"error":{"status":429,"message":"Slow"}}"#, statusCode: 429, headers: ["Retry-After": headerValue]),
        ...
        do {
            ...
            guard case .rateLimited(let seconds) = error, let s = seconds else {
    ...
    func testRateLimitRetryEventuallySucceedsForGETRequests() async throws {
        ...
        XCTAssertEqual(
            httpClient.requests.count, 3,
            "Client should retry rate-limited GET requests up to the bounded retry budget.")
    }
